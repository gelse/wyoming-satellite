...
async def main() -> None:
    ...
    settings = SatelliteSettings(
        mic=MicSettings(
            uri=args.mic_uri,
            command=['arecord', '-D', args.mic_device, '-r', str(args.mic_command_rate), '-c', str(args.mic_command_channels), '-f', args.mic_command_format, '-t', 'raw'],
        ...
        snd=SndSettings(
            uri=args.snd_uri,
            command=['aplay', '-D', args.snd_device, '-r', str(args.snd_command_rate), '-c', str(args.snd_command_channels), '-f', args.snd_command_format, '-t', 'raw'],
